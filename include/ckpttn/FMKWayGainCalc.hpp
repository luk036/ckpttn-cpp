...
public:
  ...
  FMKWayGainCalc(const Gnl &hgr, std::uint8_t num_parts)
      : hgr{hgr}, num_parts{num_parts}, rr{num_parts},
        rsrc(stack_buf, sizeof stack_buf), vertex_list{},
        delta_gain_v(num_parts, 0, &rsrc), delta_gain_w(num_parts, 0, &rsrc),
        idx_vec(&rsrc) {
...
private:
   ...
   * @param[in] weight
   */
  auto _modify_gain(const node_t &v, std::uint8_t part_v, unsigned int weight)
      -> void {
  ...
  //  * @param[in] v
  //  */
  // auto _modify_vertex_va(unsigned int weight, std::uint8_t k, const node_t&
  // v1) -> void
  // {
  ...
  //     unsigned int weight, std::uint8_t k, const node_t& v1, const node_t&
  //     v2) -> void
  // {
  //     this->vertex_list[k][v1].data.second += weight;
