...
int Vertex::to_first_vertex() {
    if (is_last_vertex()) {
        ...
        bool min_zero
            = (usteps_neg.size() == 0);  // minimum of lattice path lies on the abscissa y=0
        bool unique_min;                 // minimum of lattice path is unique
        unique_min
            = (min_zero ? (usteps_pos.front().size() == 1) : (usteps_neg.back().size() == 1));
        bool middle_level
            = (2 * count_ones() + 1 == this->bits_.size());  // vertex has same number of 0s and
        ...
        for (int d = 0; d < ((int)dsteps_neg.size()) - ((unique_min && middle_level) ? 1 : 0);
             ++d) {
            this->bits_[dsteps_neg[d].front() + 1] = 1;
        }
        // flip upsteps in the right half to downsteps
        for (int d = 0; d < ((int)usteps_neg.size()) - ((unique_min && !middle_level) ? 1 : 0);
             ++d) {
...
void Vertex::compute_flip_seq_0_rec(std::vector<int>& seq, int& idx, int left, int right,
                                    int* next_step) const {
    const int length = right - left + 1;  // total length of the subpath under consideration
...
void Vertex::compute_flip_seq_1_rec(std::vector<int>& seq, int& idx, int left, int right,
                                    int* next_step) const {
    const int length = right - left + 1;  // total length of the subpath under consideration
    if (length <= 0) {                    // base case of the recursion
        return;
