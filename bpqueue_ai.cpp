...
public:
    BPQueue(int a, int b) {
        ...
        _offset = a - 1;
        _high = b - _offset;
        _bucket = std::vector<Dllist<std::vector<int>>>(this->_high + 1, Dllist<std::vector<int>>({std::vector<int>{i, 4848}}));
    ...
    BPQueueIterator begin() {
        return BPQueueIterator(this);
    }
    BPQueueIterator end() {
        return BPQueueIterator(nullptr);
    }
};
