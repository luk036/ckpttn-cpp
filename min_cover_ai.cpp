...
class TinyGraph {
  ...
  public:
    ...
    auto neighbors(Node node) const { return adj_list[node] | std::views::keys; }
...
template <typename T> class RepeatArray {
  ...
  public:
    ...
    auto begin() const { return std::views::repeat(value).begin(); }
    auto end() const { return std::views::repeat(value, size).end(); }
};
...
class HierNetlist : public Netlist {
  ...
  public:
    ...
    size_t get_degree(Node v) const {
        return std::accumulate(ugraph.neighbors(v).begin(), ugraph.neighbors(v).end(), 0,
                               [this](size_t sum, Node net) {
                                   return sum + net_weight.count(net) ? net_weight.at(net) : 1;
                               });
    }

    size_t get_max_degree() const {
        return std::accumulate(modules.begin(), modules.end(), 0, [this](size_t max_deg, Node v) {
            return std::max(max_deg, get_degree(v));
        });
    }
    ...
    void projection_down(const std::vector<size_t>& part, std::vector<size_t>& part_down) const {
        ...
        for (size_t i_v = 0; i_v < clusters.size(); ++i_v) {
            ...
            for (Node v2 : parent->ugraph.neighbors(clusters[i_v])) {
                part_down[v2] = p;
            }
        }
    }
...
template <typename Range, typename Pred> auto filter(Range&& r, Pred&& p) {
    return std::forward<Range>(r) | std::views::filter(std::forward<Pred>(p));
}
...
std::pair<std::unordered_set<Node>, size_t> min_maximal_matching(
    ...
    auto cover = [&hyprgraph](Node net, std::unordered_set<Node>& dep) {
        for (Node vtx : hyprgraph.ugraph.neighbors(net)) {
            dep.insert(vtx);
        }
    };

    auto any_of_dep = [&hyprgraph](Node net, const std::unordered_set<Node>& dep) {
        return std::any_of(hyprgraph.ugraph.neighbors(net).begin(),
                           hyprgraph.ugraph.neighbors(net).end(),
                           [&dep](Node vtx) { return dep.count(vtx); });
    };
    ...
    for (Node net : hyprgraph.nets) {
        ...
        for (Node vtx : hyprgraph.ugraph.neighbors(net)) {
            for (Node net2 : hyprgraph.ugraph.neighbors(vtx)) {
...
std::tuple<std::vector<Node>, std::vector<Node>, std::vector<Node>> setup(
    ...
    for (Node net : hyprgraph.nets) {
        if (s1.count(net)) {
            ...
            for (Node v : hyprgraph.ugraph.neighbors(net)) {
                covered.insert(v);
            }
        } else {
    ...
    std::copy_if(hyprgraph.begin(), hyprgraph.end(), std::back_inserter(cell_list),
                 [&covered](Node v) { return !covered.count(v); });
...
std::pair<std::unordered_map<Node, size_t>, std::vector<Node>> purge_duplicate_nets(
    ...
    for (Node cluster = num_modules - num_clusters; cluster < num_modules; ++cluster) {
        for (Node net1 : ugraph.neighbors(cluster)) {
            ...
            for (Node net2 :
                 filter(ugraph.neighbors(cluster), [net1](Node n) { return n != net1; })) {
                ...
                if (ugraph.degree(net1) <= 5) {
                    ...
                    for (Node v : ugraph.neighbors(net1)) set1.insert(v);
                    for (Node v : ugraph.neighbors(net2)) set2.insert(v);
                    same = (set1 == set2);
                }
    ...
    std::copy_if(gr_nets.begin(), gr_nets.end(), std::back_inserter(updated_nets),
                 [&removelist](Node net) { return !removelist.count(net); });
...
std::tuple<TinyGraph, std::unordered_map<Node, size_t>, size_t> reconstruct_graph(
    ...
    for (size_t i_net = 0; i_net < updated_nets.size(); ++i_net) {
        ...
        for (Node v : ugraph.neighbors(net)) {
...
std::pair<HierNetlist, std::vector<size_t>> contract_subgraph(
    ...
    for (Node net : clusters) {
        node_down_list.emplace_back(*hyprgraph.ugraph.neighbors(net).begin());
    }
