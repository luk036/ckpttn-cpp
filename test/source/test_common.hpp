...
// `PM` selects the partition-manager template (FMPartMgr or NNPartMgr) so the
// FM and NN tests share a single set of assertions.

template <template <typename, typename, typename> class PM, typename GainMgr, typename ConstrMgr>
void run_PartMgr_impl(const SimpleNetlist& hyprgraph) {
    GainMgr gain_mgr{hyprgraph};
    ConstrMgr constr_mgr{hyprgraph, 0.4};
    PM<SimpleNetlist, GainMgr, ConstrMgr> part_mgr{hyprgraph, gain_mgr, constr_mgr, 2U};
...
template <template <typename, typename, typename> class PM, typename GainMgr, typename ConstrMgr>
void run_PartMgr_impl(const SimpleNetlist& hyprgraph, uint8_t num_parts) {
    GainMgr gain_mgr{hyprgraph, num_parts};
    ConstrMgr constr_mgr{hyprgraph, 0.4, num_parts};
    PM<SimpleNetlist, GainMgr, ConstrMgr> part_mgr{hyprgraph, gain_mgr, constr_mgr, num_parts};
    std::vector<uint8_t> part(hyprgraph.number_of_modules(), 0);
    auto legal_check = part_mgr.legalize(part);
    CHECK_EQ(legal_check, LegalCheck::AllSatisfied);
    auto totalcostbefore = part_mgr.total_cost;
...
void run_NNPartMgr(const SimpleNetlist& hyprgraph, uint8_t num_parts) {
    run_PartMgr_impl<NNPartMgr, GainMgr, ConstrMgr>(hyprgraph, num_parts);
}
