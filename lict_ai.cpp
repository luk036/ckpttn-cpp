...
template <typename T> class Lict : public std::map<int, T> {
  public:
    ...
    const T &operator[](int key) const { return std::map<int, T>::operator[](key); }
    typename std::map<int, T>::iterator begin() { return std::map<int, T>::begin(); }
    typename std::map<int, T>::iterator end() { return std::map<int, T>::end(); }
    typename std::map<int, T>::const_iterator begin() const { return std::map<int, T>::begin(); }
    typename std::map<int, T>::const_iterator end() const { return std::map<int, T>::end(); }
    bool __contains__(int value) const { return this->find(value) != this->end(); }
};
...
template <typename T> class ShiftArray : public std::vector<T> {
  public:
    ...
    const T &operator[](int key) const { return std::vector<T>::operator[](key - start); }
