...
template auto MLPartMgr::run_FMPartition<FMPartMgr<FMBiGainMgr, FMBiConstrMgr<SimpleNetlist>>>(
    const SimpleNetlist& H, gsl::span<std::uint8_t> part) -> LegalCheck;

template auto MLPartMgr::run_FMPartition<FMPartMgr<FMKWayGainMgr, FMKWayConstrMgr<SimpleNetlist>>>(
    const SimpleNetlist& H, gsl::span<std::uint8_t> part) -> LegalCheck;
